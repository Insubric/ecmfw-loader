...
public class API {
...

	{
		try {
			...
			HttpsURLConnection.setDefaultSSLSocketFactory(ctx
					.getSocketFactory());
			HttpsURLConnection
					.setDefaultHostnameVerifier(new IgnoreHostVerification());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private JSONObject _call(String method, String what, Object payload)
		...
		if (code != 204) {
			JSONTokener t = new JSONTokener(in);
                        ...
                        if (o.getClass().getSimpleName() == "String") {
                            System.out.println("Will give error. Try adding the ECMWF key to your home directory.\n Token: " + o);
                        }
			result = (JSONObject) o ; //t.nextValue();
	...
	private JSONObject call(String method, String what, Object payload)
			throws IOException, APIError {
		int tries = 0;
		while (true) {
			try {
	...
	public JSONObject POST(String what, Object req) throws IOException,
			APIError {
